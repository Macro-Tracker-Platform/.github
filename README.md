...
| **Infra Costs**        | :moneybag: **~100% Savings**           | Zero-cost setup via self-managed **Bare-Metal K8s** vs managed cloud solutions (AWS EKS). |
| **API Latency**        | :arrow_down: **60% Reduction**         | Parallel execution via **Reactive BFF (WebFlux)** vs sequential blocking I/O.             |
| **Docker Size**        | :arrow_down: **60% Smaller**           | Migrated Java 21 Alpine images from JDK to JRE (from ~170MB to ~70MB).                    |
